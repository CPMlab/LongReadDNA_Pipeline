...
# 2. Helper rule collecting the per-sample circos files
rule collect_sample_circos:
    ...
    shell:
        """
        # List the per-sample files that were produced
...
rule annotsv:
    ...
    shell:
        ...
        trap 'echo "--- Removing the temporary working directory ($WORK_DIR) ---" >> "$LOG_FILE_PATH_ABS"; rm -rf "$WORK_DIR"' EXIT
        
        echo "=== AnnotSV start @ $(date) ===" > "$LOG_FILE_PATH_ABS"
        ...
        echo "--- Preparing the input VCF ---" >> "$LOG_FILE_PATH_ABS"
        ...
        echo "--- Extracting AnnotSV annotations ---" >> "$LOG_FILE_PATH_ABS"
        ...
        echo "--- Running AnnotSV ---" >> "$LOG_FILE_PATH_ABS"
        ...
        if [ $AnnotSV_EXIT_CODE -ne 0 ]; then
            echo "ERROR: AnnotSV failed (exit code: $AnnotSV_EXIT_CODE)." >> "$LOG_FILE_PATH_ABS"
            exit 1
        fi

        if [ ! -s "$TmpOutputTsvInWorkDir" ]; then
            echo "ERROR: AnnotSV produced no output, or the output was empty." >> "$LOG_FILE_PATH_ABS"
            exit 1
        fi
        
        mv "$TmpOutputTsvInWorkDir" "$OUTPUT_TSV_ABS" >> "$LOG_FILE_PATH_ABS" 2>&1
        
        echo "--- AnnotSV done ---" >> "$LOG_FILE_PATH_ABS"
        echo "Final output: $OUTPUT_TSV_ABS" >> "$LOG_FILE_PATH_ABS"
        """

# 3. Intersecting SVs with IntOGen cancer genes
rule sv_intogen:
    input:
